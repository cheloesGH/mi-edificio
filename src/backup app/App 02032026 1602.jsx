...
const userFriendlyError = (errOrMsg, fallback = "Ocurrió un error. Inténtalo nuevamente.") => {
  ...
  // Conexión / fetch
  if (/failed to fetch|networkerror|fetch|TypeError/i.test(msg)) {
    return "Sin conexión a Internet o el servicio no responde. Verifica tu red e inténtalo de nuevo.";
  }

  // Payload / imagen muy grande (PostgREST / gateway)
  if (/payload too large|request entity too large|413|body too large|too large/i.test(msg)) {
    return "La imagen es muy pesada y no se pudo subir. Toma una foto más ligera o recorta la imagen e inténtalo de nuevo.";
  }
...
// Reduce tamaño de imágenes (base64) para evitar errores por payload grande.
// Convierte a JPEG, limita ancho y reduce calidad.
async function compressImageToDataURL(file, { maxWidth = 1280, quality = 0.75 } = {}) {
  if (!file) return null;
  // Si ya es pequeño, igual lo procesamos para uniformidad.
...
function ModalPago({ cuota, onClose, onConfirm }) {
  ...
  const handleImg = async (e) => {
    ...
    if (!f) return;
    try {
      const dataUrl = await compressImageToDataURL(f, { maxWidth: 1280, quality: 0.75 });
      // Si aun así es enorme, avisamos (aprox)
      if (dataUrl && dataUrl.length > 1_200_000) {
        alert("La imagen es muy pesada. Intenta con una foto más ligera o recorta la imagen.");
      }
      setImagen(dataUrl);
      setPreview(dataUrl);
    } catch (err) {
      ...
      {confirmCierre && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm space-y-4">
            ...
            <div>
              <label className="text-xs text-slate-500 mb-1 block">Soporte (imagen opcional)</label>
              <input ref={soporteRef} type="file" accept="image/*" onChange={onSoporte} className="w-full border rounded-xl px-3 py-2 text-sm bg-white" />
              {soportePreview && (
                <div className="mt-2 flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => onViewImage && onViewImage(soportePreview, "Soporte de egreso")}
                    className="flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 hover:bg-slate-50"
                    title="Ver en grande"
                  >
                    <span className="text-lg">📷</span>
                    <span className="text-sm text-slate-700">Ver</span>
                  </button>
                  <button type="button" onClick={clearSoporte} className="text-sm text-rose-600 hover:underline">Quitar soporte</button>
                </div>
              )}
            </div>
            <div className="flex gap-2">
...
function ImageViewer({ open, src, title, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/60 flex items-center justify-center p-4" onMouseDown={onClose}>
      <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden" onMouseDown={e => e.stopPropagation()}>
        <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-slate-200">
          <div className="font-semibold text-slate-800 truncate">{title || "Soporte"}</div>
          <div className="flex items-center gap-2">
            <button onClick={zoomOut} className="px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50">−</button>
            <div className="text-sm text-slate-600 w-16 text-center">{Math.round(scale * 100)}%</div>
            <button onClick={zoomIn} className="px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50">+</button>
            <button onClick={reset} className="px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50">Reset</button>
            <button onClick={onClose} className="px-3 py-1.5 rounded-lg bg-slate-900 text-white hover:bg-slate-800">Cerrar</button>
          </div>
        </div>
        <div className="bg-slate-50 max-h-[75vh] overflow-auto p-4 flex justify-center">
          {src ? (
            <img
              src={src}
              alt={title || "Soporte"}
              className="select-none rounded-xl shadow"
...
// ─── PAGOS ────────────────────────────────────────────────────────────────────
function Pagos({ pagos, setPagos, periodos, deptos, derramas, usuarios, rol, onViewImage }) {
  ...
  const registrarAbono = async (cuotaId, { monto, metodo, imagen }, isDerrama = false, cuotaVirtual = null) => {
    ...
    const montoNum = toNum(monto, NaN);
    if (!Number.isFinite(montoNum) || montoNum <= 0) { payError('Monto inválido.'); return; }
    if (isDerrama) {
      ...
      if (ex) {
        ...
        const montoPagado = parseFloat((ex.montoPagado + montoNum).toFixed(2));
        ...
        const abonos = [{ id: 1, monto: montoNum, fecha: todayStr(), metodo, imagen }];
        const montoPagado = montoNum;
      ...
      const montoPagado = parseFloat((pago.montoPagado + montoNum).toFixed(2));
            ...
            {lista.map(p => {
              ...
              return (
                <tr key={p.id} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-3 py-2.5 text-center">{lastAbono?.imagen ? (
                    <button
                      onClick={() => onViewImage && onViewImage(lastAbono.imagen, `Soporte de pago - ${p.depto}`)}
                      title="Ver soporte"
                      className="text-slate-500 hover:text-slate-700 hover:scale-125 transition-transform cursor-pointer text-lg"
                    >📷</button>
                  ) : (
                    <span className="text-slate-300 text-lg cursor-not-allowed" title="Sin soporte">📷</span>
                  )}</td>
...
// ─── EGRESOS ──────────────────────────────────────────────────────────────────
function Egresos({ egresos, setEgresos, rol, onViewImage }) {
  ...
  const onSoporte = (e) => {
    ...
    const r = new FileReader();
    r.onload = (ev) => {
      setForm(prev => ({ ...prev, soporte: ev.target.result }));
      setSoportePreview(ev.target.result);
    };
    r.readAsDataURL(f);
  };
  const clearSoporte = () => {
    setForm(prev => ({ ...prev, soporte: null }));
          ...
          <tbody>
            {filtrados.map(e => (
              <tr key={e.id} className="border-t border-slate-100 hover:bg-slate-50">
                ...
                <td className="px-4 py-3 text-center">{e.soporte ? (
                  <button
                    type="button"
                    onClick={() => onViewImage && onViewImage(e.soporte, `Soporte egreso - ${e.concepto}`)}
                    className="text-slate-500 hover:text-slate-700 hover:scale-125 transition-transform cursor-pointer text-lg"
                    title="Ver soporte"
                  >📷</button>
                ) : (
                  <span className="text-slate-300" title="Sin soporte">—</span>
                )}</td>
...
export default function App() {
  ...
  const [imgViewer, setImgViewer] = useState({ open: false, src: null, title: "" });
  const openImage = (src, title) => setImgViewer({ open: true, src, title: title || "Soporte" });
  const closeImage = () => setImgViewer(v => ({ ...v, open: false }));
    ...
    <div className="min-h-screen bg-slate-100 flex">
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        ...
        <main className="flex-1 p-4 lg:p-6 overflow-y-auto">
          ...
          {tab === "pagos" && <Pagos pagos={pagos} setPagos={setPagos} periodos={periodos} deptos={deptos} derramas={derramas} usuarios={usuarios} rol={usuario.rol} onViewImage={openImage} />}
          ...
          {tab === "egresos" && <Egresos egresos={egresos} setEgresos={setEgresos} rol={usuario.rol} onViewImage={openImage} />}
        ...
        <ImageViewer open={imgViewer.open} src={imgViewer.src} title={imgViewer.title} onClose={closeImage} />
      </div>
    </div>
  );
}
